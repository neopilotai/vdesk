...
/**
 * Client for interacting with the VDESK API.
 */
class ApiClient {
  ...
  constructor(
    ...
    if (opts?.requireApiKey && !config.apiKey) {
      throw new AuthenticationError(
        ...
        "or you can pass it directly to the sandbox like Sandbox.create({ apiKey: 'vdesk_...' })"
      )
    }
    ...
    if (opts?.requireAccessToken && !config.accessToken) {
      throw new AuthenticationError(
        ...
        'You can set the environment variable `VDESK_ACCESS_TOKEN` or pass the `accessToken` in options.'
      )
    }
