...
export interface ConnectionOpts {
  /**
   * VDESK API key to use for authentication.
   *
   * @default VDESK_API_KEY // environment variable
   */
  apiKey?: string
  /**
   * VDESK access token to use for authentication.
   *
   * @default VDESK_ACCESS_TOKEN // environment variable
   */
  accessToken?: string
  /**
   * Domain to use for the API.
   *
   * @default VDESK_DOMAIN // environment variable or `vdesk.app`
   */
  domain?: string
  /**
   * API Url to use for the API.
   * @internal
   * @default VDESK_API_URL // environment variable or `https://api.${domain}`
   */
  apiUrl?: string
  /**
   * Sandbox Url to use for the API.
   * @internal
   * @default VDESK_SANDBOX_URL // environment variable or `https://${port}-${sandboxID}.${domain}`
   */
  sandboxUrl?: string
  /**
   * If true the SDK starts in the debug mode and connects to the local envd API server.
   * @internal
   * @default VDESK_DEBUG // environment variable or `false`
   */
  debug?: boolean
  /**
...
export class ConnectionConfig {
  ...
  constructor(opts?: ConnectionOpts) {
    ...
    this.headers['User-Agent'] = `vdesk-js-sdk/${version}`
  ...
  private static get domain() {
    return getEnvVar('VDESK_DOMAIN') || 'vdesk.app'
  }

  private static get apiUrl() {
    return getEnvVar('VDESK_API_URL')
  }

  private static get sandboxUrl() {
    return getEnvVar('VDESK_SANDBOX_URL')
  }

  private static get debug() {
    return (getEnvVar('VDESK_DEBUG') || 'false').toLowerCase() === 'true'
  }

  private static get apiKey() {
    return getEnvVar('VDESK_API_KEY')
  }

  private static get accessToken() {
    return getEnvVar('VDESK_ACCESS_TOKEN')
  }
