...
describe('Template Migration', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = await fs.mkdtemp('vdesk-migrate-test-')
  })
  ...
  describe('Migration Test Cases', () => {
    ...
    async function runMigrationTest(testCaseName: string, language: Language) {
      ...
      const oldDockerfilePath = path.join(testDir, 'vdesk.Dockerfile.old')
      const oldConfigPath = path.join(testDir, 'vdesk.toml.old')
      ...
      const originalDockerfilePath = path.join(testDir, 'vdesk.Dockerfile')
      const originalConfigPath = path.join(testDir, 'vdesk.toml')
    ...
    async function copyFixtureFiles(fixtureDir: string, targetDir: string) {
      ...
      await fs.copyFile(
        path.join(fixtureDir, 'vdesk.Dockerfile'),
        path.join(targetDir, 'vdesk.Dockerfile')
      )
      await fs.copyFile(
        path.join(fixtureDir, 'vdesk.toml'),
        path.join(targetDir, 'vdesk.toml')
      )
    }
  })
  ...
  describe('Error Cases', () => {
    test('should succeed with warning when config file is missing', async () => {
      ...
      await fs.writeFile(path.join(testDir, 'vdesk.Dockerfile'), dockerfile)
      ...
      expect(output).toContain(
        'Config file ./vdesk.toml not found. Using defaults.'
      )
...
dockerfile = "vdesk.Dockerfile"`
      await fs.writeFile(path.join(testDir, 'vdesk.toml'), config)
