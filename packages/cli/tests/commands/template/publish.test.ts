...
dockerfile = "vdesk.Dockerfile"`
    ...
    test('uses the config file passed via --config even when vdesk.toml exists', async () => {
    ...
    test('uses the default vdesk.toml when --config is not provided', async () => {
        const cliPath = path.join(process.cwd(), 'dist', 'index.js')
        ...
        expect(output).toContain('vdesk.toml')
