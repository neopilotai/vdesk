...
export const imageUriMask = process.env.VDESK_IMAGE_URI_MASK
...
async function getTemplateBuildLogs({
  ...
  handleVDESKRequestError(res, 'Error getting template build status')
  return res.data as vdesk.paths['/templates/{templateID}/builds/{buildID}/status']['get']['responses']['200']['content']['application/json']
}

async function requestTemplateBuild(
  args: vdesk.paths['/templates']['post']['requestBody']['content']['application/json']
) {
...
async function requestTemplateRebuild(
  templateID: string,
  args: vdesk.paths['/templates/{templateID}']['post']['requestBody']['content']['application/json']
) {
...
async function triggerTemplateBuild(templateID: string, buildID: string) {
  ...
  handleVDESKRequestError(res, 'Error triggering template build')
  return res.data
}
...
export const buildCommand = new commander.Command('build')
  ...
  .option(
    '-d, --dockerfile <file>',
    `specify path to Dockerfile. By default VDESK tries to find ${asLocal(
      defaultDockerfileName
...
Migration guide: ${asPrimary('https://vdesk.dev/docs/template/migration-v2')}`
        ...
        if (imageUriMask == undefined) {
          try {
            child_process.execSync(
              `echo "${accessToken}" | docker login docker.${connectionConfig.domain} -u _vdesk_access_token --password-stdin`,
              {
                stdio: 'inherit',
                cwd: root,
              }
            ...
            console.error(
              'Docker login failed. Please try to log in with `vdesk auth login` and try again.'
            )
            process.exit(1)
          }
        }
  ...
  do {
    ...
    switch (template.status) {
      case 'building':
        break
      case 'ready': {
        const pythonExample = asPython(`from vdesk import Sandbox, AsyncSandbox
...
sandbox = await AsyncSandbox.create("${aliases?.length ? aliases[0] : template.templateID
        ...
        const typescriptExample = asTypescript(`import { Sandbox } from 'vdesk'
...
export function getDockerfile(root: string, file?: string) {
  ...
  // Check if default dockerfile vdesk.Dockerfile exists
...
async function requestBuildTemplate(
  ...
  Omit<
    vdesk.paths['/templates']['post']['responses']['202']['content']['application/json'],
    'logs'
  >
> {
  let res
  ...
  handleVDESKRequestError(res, 'Error requesting template build')
  return res.data
}
...
function dockerImageUrl(
  ...
  if (imageUrlMask == undefined) {
    return `docker.${defaultDomain}/vdesk/custom-envs/${templateID}:${buildID}`
  }
