...
function renderTable(templates: vdesk.components['schemas']['Template'][]) {
  if (!templates?.length) {
...
export async function listSandboxTemplates({
  ...
  teamID?: string
}): Promise<vdesk.components['schemas']['Template'][]> {
  ...
  handleVDESKRequestError(templates, 'Error getting templates')
