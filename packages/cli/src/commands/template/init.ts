...
async function generateTemplateFiles(
  ...
  const template = Template().fromBaseImage().runCmd('echo Hello World VDESK!')
...
.PHONY: vdesk:build:dev
vdesk:build:dev:
\t${cdPrefix}python ${files.buildDevFile}

.PHONY: vdesk:build:prod
vdesk:build:prod:
...
`
    ...
    console.log(
      `   ${asPrimary('make vdesk:build:dev')} - Build development template`
    )
    console.log(
      `   ${asPrimary('make vdesk:build:prod')} - Build production template`
    )
  } catch (err) {
    console.warn(
  ...
  try {
    ...
    pkgJson.update({
      scripts: {
        ...
        'vdesk:build:dev': `${cdPrefix}npx tsx ${files.buildDevFile}`,
        'vdesk:build:prod': `${cdPrefix}npx tsx ${files.buildProdFile}`,
      },
    })
    ...
    console.log(
      `   ${asPrimary('npm run vdesk:build:dev')} - Build development template`
    )
    console.log(
      `   ${asPrimary('npm run vdesk:build:prod')} - Build production template`
    )
  } catch (err) {
    console.warn(
    ...
    async (opts: { path?: string; name?: string; language?: Language }) => {
      try {
        ...
        switch (language) {
          case Language.TypeScript:
            console.log(
              `   ${asPrimary('npm install vdesk')} (install vdesk dependency)`
            )
            console.log(
              `   ${asPrimary('npm run vdesk:build:dev')} (for development)`
            )
            console.log(
              `   ${asPrimary('npm run vdesk:build:prod')} (for production)`
            )
            break
          case Language.PythonAsync:
          case Language.PythonSync:
            console.log(
              `   ${asPrimary('pip install vdesk')} (install vdesk dependency)`
            )
            console.log(
              `   ${asPrimary('make vdesk:build:dev')} (for development)`
            )
            console.log(
              `   ${asPrimary('make vdesk:build:prod')} (for production)`
            )
            break
          default:
