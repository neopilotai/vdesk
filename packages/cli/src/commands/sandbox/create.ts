  ...
  return new commander.Command(name)
    ...
    .action(
      async (
        ...
        if (deprecated) {
          console.warn(
            `Warning: The '${name}' command is deprecated and will be removed in future releases. Please use 'vdesk sandbox create' instead.`
          )
        }
        try {
          ...
          const sandbox = await vdesk.Sandbox.create(templateID, { apiKey })
