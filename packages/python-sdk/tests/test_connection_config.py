...
def test_api_url_defaults_correctly(monkeypatch):
    monkeypatch.setenv("VDESK_DOMAIN", "")

    config = ConnectionConfig()
    assert config.api_url == "https://api.vdesk.app"
...
def test_api_url_in_env_var(monkeypatch):
    monkeypatch.setenv("VDESK_API_URL", "http://localhost:8080")
...
def test_api_url_has_correct_priority(monkeypatch):
    monkeypatch.setenv("VDESK_API_URL", "http://localhost:1111")
