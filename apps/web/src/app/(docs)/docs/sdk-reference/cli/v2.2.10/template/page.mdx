...
### Options
 ...
 - `-d, --dockerfile <file>: specify path to Dockerfile. By default E2B tries to find vdesk.Dockerfile or Dockerfile in root directory. `
 ...
 - `-t, --team <team-id>: specify the team ID that the operation will be associated with. You can find team ID in the team settings in the E2B dashboard (https://vdesk.dev/dashboard?tab=team). `
...
create basic E2B Dockerfile (./vdesk.Dockerfile) in root directory. You can then run vdesk template build to build sandbox template from this Dockerfile
