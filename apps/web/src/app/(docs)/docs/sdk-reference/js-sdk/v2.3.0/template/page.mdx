...
Start from VDESKSK's default base image (vdeskdev/base:latest).
...
Start from an existing VDESKSK template.

###### Parameters

| Parameter | Type | Description |
| ------ | ------ | ------ |
| `template` | `string` | VDESKSK template ID or alias |
...
Error if the template is based on another VDESKSK template
...
| ------ | ------ | ------ |
| `apiKey`? | `string` | VDESKSK API key for authentication. |
| `domain`? | `string` | Domain of the VDESKSK API. |
...
Create a new VDESKSK template builder instance.
