...
Builder class for adding instructions to an VDESKSK template.
...
Base class for building VDESKSK sandbox templates.
...
Start template from the VDESKSK base image (vdeskdev/base:latest).
...
Start template from an existing VDESKSK template.

**Arguments**:

- `template`: VDESKSK template ID or alias
...
Note: Templates based on other VDESKSK templates cannot be converted to Dockerfile.
...
- `ValueError`: If the template is based on another VDESKSK template or has no base image
Example
```python
template = Template().from_python_image('3').copy('app.py', '/app/')
...
Internal representation of a template for the VDESKSK build API.
